/**
 * Heuristic extractor for document validation.
 *
 * Shared by the Word plugin (/api/word/*) and Google Docs plugin
 * (/api/gdocs/*) validate/finalize endpoints. This is intentionally simple:
 * regex + keyword scanning on plaintext to produce a best-effort structured
 * payload. Final validation is performed by the canonical zod schemas in
 * `@glyph/schema-library` — any fields the heuristic cannot infer are
 * returned as `missingFields` so the client can prompt the user.
 *
 * Zero `any`. Defensive: if the doc is gibberish we return `valid: false`
 * and the list of fields that still need values.
 */
...
function extractContract(text: string): HeuristicResult {
  ...
  // Parties: look for "between X and Y" or bullet list with role markers.
